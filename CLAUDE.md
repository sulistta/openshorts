...
- Durable projects and gallery assets live below `output/`.
...
- Temporary uploads may be cleaned up to enforce disk caps. Durable projects
  and gallery records must not expire automatically.
...
remote-storage assumptions to these contracts. Gallery reads are public;
deletion requires an explicit `confirm=true` query parameter.

## Useful commands

```bash
python3 -m py_compile app.py mcp_server.py local_library.py local_gallery.py
