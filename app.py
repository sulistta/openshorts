...
async def cleanup_jobs():
    ...
    while True:
        try:
            ...
            # Durable projects and gallery entries are never expired. Only
            # transient work directories are subject to this sweep.
            for job_id in os.listdir(OUTPUT_DIR):
                if job_id in {"gallery", "thumbnails"}:
                    continue
                job_path = os.path.join(OUTPUT_DIR, job_id)
            ...
            # Cleanup transient SaaSShorts jobs from memory
            try:
                saas_expired = [
...
@app.get("/api/gallery/actors")
async def get_gallery_actors(limit: int = 200):
    return {"actors": list_actors(OUTPUT_DIR)[:max(1, min(limit, 500))]}


@app.get("/api/gallery/videos")
async def get_gallery_videos(limit: int = 200):
    return {"videos": list_videos(OUTPUT_DIR, limit=max(1, min(limit, 500)))}


@app.delete("/api/gallery/actors/{actor_id}")
async def remove_gallery_actor(actor_id: str, confirm: bool = False):
    if not confirm:
        raise HTTPException(status_code=400, detail="Pass confirm=true to delete this actor")
    if not delete_actor(OUTPUT_DIR, actor_id):
        raise HTTPException(status_code=404, detail="Actor not found")
    return {"deleted": True, "actor_id": actor_id}


@app.delete("/api/gallery/videos/{video_id}")
async def remove_gallery_video(video_id: str, confirm: bool = False):
    if not confirm:
        raise HTTPException(status_code=400, detail="Pass confirm=true to delete this video")
    if not delete_video(OUTPUT_DIR, video_id):
...
async def saasshorts_actor_upload(request: Request, file: UploadFile = File(...)):
    """Upload a custom actor image into the local gallery."""
    ...
    try:
        ...
        actor = save_actor(OUTPUT_DIR, file_path, description="Uploaded actor")
        if not actor:
            raise HTTPException(status_code=500, detail="Could not persist actor image")
        # The gallery owns its durable copy; remove the temporary upload so
        # deleting the actor does not leave an undiscoverable duplicate.
        try:
            os.remove(file_path)
        except OSError:
            pass
        return {"url": actor["url"], "actor": actor}
...
async def saasshorts_actor_options(
    ...
    try:
        ...
        # Copy each generated actor into the durable local gallery.
        desc = req.actor_description
        if req.product_description:
            desc += f" (holding {req.product_description})"
        urls = []
        for p in paths:
            actor = save_actor(OUTPUT_DIR, p, description=desc)
            if actor:
...
async def saasshorts_video_gallery(limit: int = 50):
    ...
    try:
        loop = asyncio.get_running_loop()
        videos = await loop.run_in_executor(None, list_videos, OUTPUT_DIR, limit)
...
async def gallery_html_page():
    """Public gallery page with all generated UGC videos."""
    import html as html_mod
    loop = asyncio.get_running_loop()
    videos = await loop.run_in_executor(None, list_videos, OUTPUT_DIR, 100)
    ...
    for i, v in enumerate(videos):
        ...
        ld_items.append(f'{{"@type":"ListItem","position":{i+1},"url":"/video/{video_id}","name":"{title}"}}')
...
async def video_html_page(video_id: str):
    """Public individual video page with og:video metadata."""
    import html as html_mod
    loop = asyncio.get_running_loop()
    videos = await loop.run_in_executor(None, list_videos, OUTPUT_DIR, 200)
...
async def saasshorts_actor_gallery():
    """List all previously generated actor images from the local gallery."""
    try:
        loop = asyncio.get_running_loop()
        images = await loop.run_in_executor(None, list_actors, OUTPUT_DIR)
...
async def saasshorts_generate(
    ...
    if req.selected_actor_url:
        if req.selected_actor_url.startswith("http"):
            # Download a selected public actor URL to the job output directory.
            import httpx
            ...
            if job_id in saas_jobs:
                ...
                if req.share_to_gallery:
                    try:
                        ...
                        gallery_result = save_video(
                            OUTPUT_DIR,
                            video_path=result["video_path"],
                            actor_path=result.get("actor_image", ""),
