...
export default function SaaShortsTab({ geminiApiKey, elevenLabsKey, falKey, uploadPostKey, uploadUserId }) {
  ...
  const needsGeminiKey = !geminiApiKey;
  // Wizard state
  ...
  useEffect(() => {
    ...
    fetch(getApiUrl('/api/gallery/actors'))
      .then(res => res.ok ? res.json() : { actors: [] })
      .then(data => setActorGallery(data.actors || []))
      .catch(() => {})
                  ...
                  <div className="mb-4">
                    ...
                    <div className="grid grid-cols-4 sm:grid-cols-6 gap-2 max-h-48 overflow-y-auto pr-1">
                      {actorGallery.map((img, i) => (
                        ...
                        >
                          <img src={getApiUrl(img.url)} alt={`Actor ${i+1}`} className="w-full h-full object-cover" />
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      if (res.ok) {
                        ...
                        const galRes = await fetch(getApiUrl('/api/gallery/actors'));
                        if (galRes.ok) {
                          const galData = await galRes.json();
                          setActorGallery(galData.actors || []);
                        }
                      }
                  ...
                  <div className="mt-3">
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      {actorOptions.map((imgUrl, i) => (
                        ...
                        >
                          <img src={getApiUrl(imgUrl)} alt={`New ${i+1}`} className="w-full h-full object-cover" />
              ...
              <label className="flex items-start gap-2 text-sm text-ink cursor-pointer">
                ...
                <span>
                  ...
                  <span className="block text-xs text-muted">
                    Your video, product name and script will be visible on this instance's public gallery
                  </span>
                </span>
              </label>
            </div>
